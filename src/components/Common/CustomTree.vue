<template>
  <li :class="[isButton, hasBorder]" style="list-style:none;">
    <span v-show="model.menuLevel!==1" @click="toggle">
      <i v-if="isFolder" class="icon" :class="[open ? 'folder-open': 'folder']" style="margin-bottom: 3px;" />
      <i v-if="!isFolder" class="icon file-text" />
      <input :id="'menu'+model.id" type="checkbox" class="checkCls" :class="'group'+label" @click.stop="selTree(model)"> {{ model.menuName }}
    </span>
    <ul v-show="open" v-if="isFolder">
      <tree-menu
        v-for="(item, index) in model.childNode"
        :key="index"
        :model="item"
        :menu-list="menuList"
        :label="label"
...
export default {
  ...
  mounted() {
    ...
    // 选中菜单使能
    if (
      this.selectKeys instanceof Array &&
      this.selectKeys.length > 0 &&
      this.selectKeys.indexOf(this.model.id) >= 0
    ) {
      if (
        this.model.childNode &&
        this.model.childNode.length &&
        this.model.menuLevel !== 1
      ) {
        // 包含子级,一级菜单除外
        // 计算所有子节点是否全部选中
      ...
      if (allMenuList.id === id) {
        ...
        for (let i = 0; i < allMenuList.childNode.length; i++) {
          if (allMenuList.childNode[i].id === id) {
            ...
            allMenuList.childNode[i].childNode &&
            allMenuList.childNode[i].childNode.length
          ) {
            // 三级
            for (
              let j = 0;
              j < allMenuList.childNode[i].childNode.length;
              j++
            ) {
