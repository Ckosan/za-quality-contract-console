...
Vue.filter('GET_LABEL_THROUGH_VALUE_LOG', function(value, localArray) {
  if (!localArray) return value
  const temp = localArray.filter(item => item.menu_code == value)

  return temp && temp[0] ? temp[0].menu_desc : value
})
Vue.filter('GET_LABEL_THROUGH_VALUE_CODE', function(value, localArray) {
  if (!localArray) return value
  const temp = localArray.filter(item => item.errorCode == value)

  return temp && temp[0] ? temp[0].errorDesc : value
})
